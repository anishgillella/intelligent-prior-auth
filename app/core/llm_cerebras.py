"""
Cerebras LLM client implementation using OpenAI-compatible API
...
from app.core.monitoring import LLMCallMonitor, get_langfuse_client

logger = logging.getLogger(__name__)


class CerebrasLLMClient(BaseLLMClient):
    """Cerebras LLM client for fast inference"""
    
    # Pricing per 1M tokens (as of 2024)
    PRICING = {
        "gpt-oss-120b": {"input": 0.35, "output": 0.75},
        "llama-3.1-70b": {"input": 0.35, "output": 0.75},
        "gpt-4": {"input": 0.30, "output": 0.60},
    }
    
    def __init__(self):
        """Initialize Cerebras LLM client using OpenAI-compatible API"""
        try:
            import openai
        ...
        logger.info(f"Cerebras LLM Client initialized with model: {self.model}")
    ...
    def call(
        ...
        """
        Call Cerebras API (OpenAI-compatible endpoint) with retry logic
        ...
        try:
            # Create OpenAI-compatible client pointing to Cerebras API
            import httpx
            ...
            logger.info(
                f"Cerebras LLM call successful: {self.model} | "
